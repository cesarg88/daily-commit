...
export default async function AppLayout({
  ...
  return (
    <main className="min-h-screen px-4 py-6 sm:px-6 sm:py-8">
      <header className="mx-auto flex w-full max-w-5xl flex-col gap-4 border-b border-neutral-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-3 sm:items-end">
          ...
          <form action="/logout" method="post">
            <button
              className="w-full border border-neutral-300 bg-white px-3 py-2 text-sm font-medium sm:w-auto"
              type="submit"
            >
              Sign out
            </button>
      ...
      <div className="mx-auto w-full max-w-5xl py-6 sm:py-8">{children}</div>
    </main>
  );
}
